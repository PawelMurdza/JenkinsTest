...
Background:
  ...
  Scenario:Adding an employee
    ...
    Scenario: Get the newly created employee
      ...
      And the employee we are getting having ID "employee.employee_id" must match with globally stored employee id
      ...
      |Tom          |Green       |John           |Male      |2000-06-20  |Probation |QA           |


      @jsonpayload
     Scenario: Adding an employee via json payload
        ...
        @dynamic
        Scenario:
          Given a request is prepared to create an employee via dynamic payload "Tom" ,"Green","John","M","2000-06-20","Probation","QA"
          When a POST call is made to create an employee
          Then the status code for the created employee is 201
          And the employee created contains key "Message" and value "Employee Created"
          And the employee id "Employee.employee_id" is stored as a global variable to be used for other calls
